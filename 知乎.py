...
def Download(q,Topic,path,ShareData):
	...
	try:
		while not q.empty():
			...
			r.raise_for_status()
			path1=os.path.join(path,Topic+url[-10:])
...
def main():
	...
	Topic=input('Topic ')
	pages=input('pages ')
	imgLis=getUrls(Topic,pages)
	q=Queue()
	putUrl(q,imgLis)
	path='D:\\python_pic'+'\\'+Topic
	if os.path.exists(path)==False:
		os.makedirs('D:\\python_pic'+'\\'+Topic)
		path='D:\\python_pic'+'\\'+Topic
